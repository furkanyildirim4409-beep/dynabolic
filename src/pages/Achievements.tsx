...
const Achievements = () => {
  ...
  // Trigger confetti for newly viewed unlocked badges
  const handleBadgeClick = (badge: Achievement) => {
    ...
    if (badge.unlocked && !celebratedBadges.has(badge.id)) {
      hapticSuccess();
      setCelebratedBadges(prev => new Set([...prev, badge.id]));
      ...
      confetti({
        ...
        spread: 70,
        origin: { y: 0.6 },
        colors: ['#a3e635', '#22c55e', '#facc15', '#f97316'],
      });
    }
  };
        ...
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-xs">
              {userGamificationStats.unlockedBadges}/{userGamificationStats.totalBadges} rozet açıldı
            </p>
          </div>
        </div>
          ...
          <div className="glass-card p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              {nextTier && (
                <div className="flex items-center gap-1.5 text-muted-foreground text-xs">
                  <span>{userGamificationStats.currentXP} XP</span>
                  <span>/</span>
            ...
            <div className="h-2 bg-secondary rounded-full overflow-hidden">
              <motion.div
                ...
                className={`h-full bg-gradient-to-r ${currentTier.gradient} rounded-full`}
              />
            </div>
            ...
            {nextTier && (
              <div className="flex items-center justify-between mt-2">
                ...
                <div className="flex items-center gap-1">
                  <nextTier.icon className={`w-3 h-3 ${nextTier.color}`} />
                  <span className={`text-[10px] ${nextTier.color}`}>{nextTier.name}</span>
                </div>
              </div>
            )}
          </div>
        ...
        {filteredAchievements.map((badge, index) => {
          ...
          return (
            <motion.button
              ...
              className={`relative aspect-square rounded-2xl border p-3 flex flex-col items-center justify-center gap-2 transition-all ${
                badge.unlocked
            ...
            >
              ...
              {/* Icon */}
              <div className={`relative ${badge.unlocked ? '' : 'opacity-30'}`}>
                <IconComponent 
                  className={`w-8 h-8 ${badge.unlocked ? colors.text : 'text-muted-foreground'}`} 
                />
                {!badge.unlocked && (
              ...
              {!badge.unlocked && badge.progress !== undefined && (
                <div className="absolute bottom-2 left-2 right-2 h-1 bg-secondary rounded-full overflow-hidden">
                  <div 
              ...
              <div className={`absolute top-1.5 right-1.5 w-2 h-2 rounded-full ${
                badge.unlocked
                  ? badge.tier === 'platinum' ? 'bg-purple-400'
            ...
            <motion.div
              ...
              className={`w-full max-w-sm rounded-3xl border p-6 ${
                selectedBadge.unlocked
                  ? `${tierColors[selectedBadge.tier].bg} ${tierColors[selectedBadge.tier].border}`
            ...
            >
              ...
              <div className="flex justify-center mb-4">
                <motion.div
                  ...
                  transition={{ duration: 1, repeat: selectedBadge.unlocked ? Infinity : 0, repeatDelay: 2 }}
                  className={`w-24 h-24 rounded-2xl flex items-center justify-center ${
                    selectedBadge.unlocked
                      ? `bg-gradient-to-br ${tierColors[selectedBadge.tier].bg} border-2 ${tierColors[selectedBadge.tier].border}`
                ...
                >
                  <selectedBadge.icon 
                    className={`w-12 h-12 ${
                      selectedBadge.unlocked 
                        ? tierColors[selectedBadge.tier].text 
              ...
              <div className="glass-card p-3 mb-4">
                <p className="text-xs text-muted-foreground mb-1">GEREKSINIM</p>
              ...
              {selectedBadge.unlocked ? (
                <div className="flex items-center justify-center gap-2 text-primary">
                  ...
                  <span className="text-sm">
                    {selectedBadge.unlockedAt && new Date(selectedBadge.unlockedAt).toLocaleDateString('tr-TR')} tarihinde açıldı
                  </span>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs text-primary">{selectedBadge.progressText}</span>
                  </div>
                  <div className="h-2 bg-secondary rounded-full overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
              ...
              <div className="mt-4 text-center">
                <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium ${
                  selectedBadge.unlocked ? 'bg-primary/20 text-primary' : 'bg-secondary text-muted-foreground'
                }`}>
