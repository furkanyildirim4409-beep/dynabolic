...
const Beslenme = () => {
  ...
  const totalMacros = meals.reduce(
    (acc, meal) => ({
      p: acc.p + meal.totalMacros.p,
      c: acc.c + meal.totalMacros.c,
      f: acc.f + meal.totalMacros.f,
    }),
    { p: 0, c: 0, f: 0 },
  );

  // Hedef Makrolar (Örnek)
  const macroGoals = { p: 180, c: 250, f: 70 };
  ...
  return (
    <div className="min-h-screen bg-black px-4 pt-6 pb-32">
      ...
      <div className="flex flex-col gap-4 mb-6">
        ...
        {/* --- YENİ EKLENEN MAKRO TABLOSU --- */}
        <div className="grid grid-cols-3 gap-3">
          {/* Protein Card */}
          <div className="bg-[#1a1a1a] border border-white/5 rounded-xl p-3 flex flex-col gap-2">
            ...
            <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${Math.min((totalMacros.p / macroGoals.p) * 100, 100)}%` }}
              />
            </div>
          </div>
          ...
          <div className="bg-[#1a1a1a] border border-white/5 rounded-xl p-3 flex flex-col gap-2">
            ...
            <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${Math.min((totalMacros.c / macroGoals.c) * 100, 100)}%` }}
              />
            </div>
          </div>
          ...
          <div className="bg-[#1a1a1a] border border-white/5 rounded-xl p-3 flex flex-col gap-2">
            <div className="flex justify-between items-center text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
              <span>Yağ</span>
              <span className="text-white">{totalMacros.f}g</span>
            </div>
            <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
              <div
                className="h-full bg-red-500 rounded-full"
                style={{ width: `${Math.min((totalMacros.f / macroGoals.f) * 100, 100)}%` }}
              />
            </div>
          </div>
        </div>

        {/* Action Buttons - İsim ve İkon Değişti */}
