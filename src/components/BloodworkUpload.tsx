  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validTypes.includes(file.type)) {
      toast({
        ...
        description: "Lütfen PDF veya resim dosyası yükleyin.",
        variant: "destructive",
      });
      return;
    }
      ...
      setUploadProgress((prev) => {
        if (prev >= 100) {
          ...
          toast({
            ...
            description: "Kan tahlili koçunuza gönderildi. Analiz sonuçlarını bekleyin.",
          });
          return 0;
        }
        return prev + 10;
      ...
      <AnimatePresence>
        {isUploading && (
          <motion.div
            ...
            className="mb-4 p-4 glass-card border border-primary/30"
          >
            <div className="flex items-center gap-3 mb-3">
              <Upload className="w-5 h-5 text-primary animate-pulse" />
              <span className="text-sm text-foreground">Yükleniyor...</span>
              <span className="text-sm text-primary ml-auto">{uploadProgress}%</span>
            </div>
                    ...
                    <div className="p-4 space-y-3">
                      {report.analysisDate && (
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="text-foreground">{formatDate(report.analysisDate)}</span>
                        </div>
                      )}
                      ...
                      {report.coachNotes && (
                        <div className="p-3 bg-primary/10 border border-primary/20 rounded-xl">
                          ...
                          <p className="text-sm text-foreground">{report.coachNotes}</p>
                        </div>
                      )}
        ...
        {bloodworkReports.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground text-sm">
              Henüz kan tahlili yüklenmemiş.
            </p>
            <p className="text-muted-foreground/70 text-xs mt-1">
              İlk tahlilinizi yüklemek için yukarıdaki butonu kullanın.
            </p>
          </div>
        )}
      </div>
