...
const StoriesRing = ({ className = "" }: StoriesRingProps) => {
  ...
  const handleCategoryClick = (category: StoryCategory) => {
    ...
    // Mevcut kategorinin sırasını bul
    const currentIndex = categories.indexOf(category);
    // Bir sonraki kategoriyi belirle
    const nextCategory = categories[currentIndex + 1];

    // Eğer sonraki kategori varsa, bu kategori bittiğinde çalışacak fonksiyonu hazırla
    ...
    openStories(stories, 0, {
      ...
      onComplete: onComplete, // Viewer'a "bitince bunu yap" diyoruz
    });
