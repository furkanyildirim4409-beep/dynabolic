...
interface DisputeResolutionModalProps {
  ...
  challengeType?: "pr" | "streak";
  exercise?: string;
  onSubmitAppeal?: (appealReason: string) => void;
  onSendMessage?: (disputeId: string, message: string) => void;
}

// Mock coach/admin data
const mockReviewer = {
  id: "coach-serdar",
  name: "Koç Serdar",
...
const DisputeResolutionModal = ({
  ...
  const handleSubmitAppeal = async () => {
    if (!appealReason.trim()) {
      toast({
        title: "İtiraz sebebi gerekli",
        description: "Lütfen neden itiraz ettiğinizi açıklayın",
        variant: "destructive",
      });
      return;
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-display text-base text-foreground">
                {isNewDispute ? "İtiraz Gönder" : "İtiraz Detayı"}
              </h2>
              <p className="text-muted-foreground text-xs">
                {isNewDispute ? "Reddedilen kanıtınız için itiraz edin" : `#${dispute?.id.slice(0, 8)}`}
              </p>
            </div>
          </div>
            ...
            <div className="glass-card p-3 space-y-3">
              ...
              <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/20">
                ...
                <p className="text-foreground text-sm">
                  {dispute?.originalRejectionReason || rejectionReason}
                </p>
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Kanıtınızın neden geçerli olduğunu açıklayın..."
                    className="bg-secondary border-white/10 min-h-[120px]"
                  />
                  <p className="text-muted-foreground text-[10px]">
                    Koçunuz itirazınızı inceleyecek ve karar verecek
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <button
                    onClick={() => {
                      hapticLight();
                      // Mock additional evidence upload
                      setAdditionalEvidence("https://example.com/evidence.jpg");
                      toast({ title: "Ek kanıt eklendi" });
                    }}
                  ...
                  >
                    ...
                    <span className="text-sm">Farklı açıdan fotoğraf veya video yükle</span>
                  </button>
                  {additionalEvidence && (
                  ...
                  <div className="glass-card p-3">
                    <div className="flex items-center gap-3">
                      <Avatar className="w-10 h-10 ring-2 ring-primary">
                        <AvatarImage src={mockReviewer.avatar} className="object-cover" />
                        <AvatarFallback className="bg-primary/20 text-primary">KS</AvatarFallback>
                      </Avatar>
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="px-1.5 py-0.5 rounded-full bg-primary/20 text-primary text-[9px] font-medium">
                            {dispute.reviewerRole === "admin" ? "Admin" : "Koç"}
                          </span>
                        </div>
                        <p className="text-muted-foreground text-[10px]">
                          {dispute.status === "under_review" 
                            ? "İnceleniyor..." 
                            : `Karar verildi: ${formatDate(dispute.reviewedAt!)}`
                          }
                        </p>
                      </div>
                    ...
                    {dispute.resolution && (
                      <div className={`mt-3 p-3 rounded-xl ${
                        ...
                        <div className={`flex items-center gap-2 text-xs mb-1 ${
                          dispute.status === "resolved_approved" ? "text-emerald-400" : "text-red-400"
                        }`}>
                        ...
                        <motion.div
                          ...
                          className={`p-3 rounded-xl ${
                            msg.senderId === "current"
                              ...
                              : msg.senderRole === "coach" || msg.senderRole === "admin"
                              ? "bg-amber-500/10 border border-amber-500/20 mr-6"
                              : "bg-secondary mr-6"
                          }`}
                        >
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {msg.senderRole !== "athlete" && (
                              <span className="px-1 py-0.5 rounded bg-amber-500/20 text-amber-400 text-[8px]">
                                {msg.senderRole === "admin" ? "Admin" : "Koç"}
                              </span>
                            )}
                ...
                {!dispute.status.startsWith("resolved") && (
                  <div className="flex gap-2">
                    <Textarea
                      ...
                      className="bg-secondary border-white/10 min-h-[60px] flex-1"
                    />
                    <Button
